# PaCT: Parallel Continuous Thought for Reasoning in Large Language Models

This repository contains the official implementation for our ICML 2026 submission:

**PaCT: Parallel Continuous Thought for Reasoning in Large Language Models**

<p align="center">
  <img src="assets/compare_pact_coconut.png" width="600">
</p>

## Overview

PaCT introduces parallel continuous thought generation for reasoning in large language models. Unlike sequential approaches, PaCT generates multiple latent thoughts in parallel using a cross-attention mechanism with learnable queries and hence faster. 

<p align="center">
  <img src="assets/intro_diagram.png" width="800">
</p>
...
Setup environment:
```
conda create --name pact python=3.12
conda activate pact
...
The configuration of a run should be specified in a yaml file (an example can be found [here](args/gsm_pact.yaml)).
...
- **Method**
  - **pact**: Train PaCT model
  - **cot**: Train CoT model
  - **no_thoughts**: Train PaCT (w/o thought) model
  - **no_cot**: Train no-CoT model

- **Training settings**

  - **c_thought**: Number of continuous thoughts (latents) for each reasoning step
  - **num_context_tokens**: Number of context tokens used as keys/values in cross-attention
  - **projection_rank**: Rank for low-rank projection in the latent space
  - **codebook_heads**: Number of attention heads in the cross-attention module
  ...
  - **save_only_improve**: Save the model only when there the best validation accuracy is updated. Recommended to set `False` for PaCT model training, because otherwise the checkpoints in the last stage might not get saved.
  ...
  - **load_model_path**: The path to a checkpoint to load. Used in two cases: (1) for evaluation (2) to initialize PaCT from a CoT-tuned model.
  - **seed**: Random seed.
...
All the commands below assume 4 * H100 (80GB) GPUs. You may change the corresponding arguments in the config file (`batch_size_training`, `gradient_accumulation_steps`) and `nproc_per_node` when launching the run, to adapt your resources.
...
Select a checkpoint as the initialization of PaCT (the validation accuracy is expected to be around 40%). Replace the `load_model_path` in the [args/gsm_pact.yaml](args/gsm_pact.yaml) with your selected checkpoint, and run:

```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/gsm_pact.yaml
```

Find the checkpoint with best validation accuracy, and put the path as `load_model_path` in [args/gsm_pact_eval.yaml](args/gsm_pact_eval.yaml). To evaluate:

```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/gsm_pact_eval.yaml
```
...
```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/prontoqa_pact.yaml
```

Find the checkpoint with best validation accuracy, and put the path as `load_model_path` in [args/prontoqa_pact_eval.yaml](args/prontoqa_pact_eval.yaml). To evaluate:

```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/prontoqa_pact_eval.yaml
```
...
Then run the following to train the model:
```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/prosqa_pact.yaml
```

Find the checkpoint with best validation accuracy, and put the path as `load_model_path` in [args/prosqa_pact_eval.yaml](args/prosqa_pact_eval.yaml). To evaluate:

```bash
torchrun --nnodes 1 --nproc_per_node 4 run.py args/prosqa_pact_eval.yaml
```

# Acknowledgements
This codebase was built on top of the original codebase of [Coconut](https://github.com/facebookresearch/coconut)