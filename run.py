...
def main():
    ...
    parser = argparse.ArgumentParser(description="pact")
    ...
    if configs.load_model_path is not None and configs.load_model_path != "None":
        print(f"Loading model from {configs.load_model_path}")
        try:
            ...
            if rank == 0:
                print(f"ERROR: Failed to load checkpoint from {configs.load_model_path}")
                print(f"Error: {e}")
                print("This usually means the checkpoint file is corrupted or incomplete.")
                print("Possible solutions:")
                print("  1. Set load_model_path: None to start from scratch")
                print("  2. Use a different checkpoint file")
                print("  3. Check if the checkpoint file was interrupted during saving")
            raise

        if configs.pact and not any(
            [k.startswith("base_causallm") for k in saved_weights.keys()]
        ):
            # we are loading a base model into pact model
            # e.g., for GSM8k, we used a SFTed model to skip the stage 0
            loaded = True
            print(model.load_state_dict(saved_weights, strict=False))

        elif not configs.pact and any(
            [k.startswith("base_causallm") for k in saved_weights.keys()]
        ):
            raise ValueError("Cannot load pact model weights into a causallm model")

        elif configs.pact and any(
            [k.startswith("base_causallm") for k in saved_weights.keys()]
        ):
    ...
    if hasattr(configs, 'use_lora') and configs.use_lora:
        if rank == 0:
            print(f"Applying LoRA with rank={configs.lora_r}, alpha={configs.lora_alpha}")
    ...
    if configs.load_model_path is not None and configs.load_model_path != "None" and not loaded:
        ...
        # However, if we are resuming from a checkpoint that didn't have projection heads (e.g. old code),
        # strict=False allows us to proceed, but we should be aware that projection heads are random init.
        # If we are resuming a run that HAD projection heads, they should be in saved_weights.
        
        # The error shows missing keys for 'latent_projections'. This means the checkpoint 
        # does NOT contain these keys. This is expected if we are starting from a checkpoint 
        # trained with the OLD architecture.
    ...
    if not configs.debug and not configs.only_eval and rank == 0:
        ...
        wandb_run = wandb.init(project=configs.project, name=run_name)
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            ...
            if configs.reset_optimizer and epoch % configs.epochs_per_stage == 0 and epoch // configs.epochs_per_stage < configs.max_latent_stage:
                del optimizer
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                samples_per_sec = current_batch_size / (step_duration + 1e-6)

                if wandb_run and rank == 0:
                    log_dict = {
                        "train/epoch": epoch + 1,
                        "train/step": epoch * len(train_dataloader) + step,
                        "train/loss": outputs.loss.detach().float(),  # Log raw CE loss
                ...
                pbar.set_description(
                    ...
                    f"loss: {round(float(outputs.loss.detach().float()), 4)} spd: {round(samples_per_sec, 2)} s/s"
                )
            pbar.close()
            dist.barrier()
        ...
        eval_frequency = getattr(configs, 'eval_frequency', 1)
        should_evaluate = (epoch + 1) % eval_frequency == 0 or (epoch + 1) == configs.num_epochs or configs.only_eval
        ...
        if should_evaluate:
            ...
            if rank == 0:
                print(f"Skipping evaluation at epoch {epoch+1}")
