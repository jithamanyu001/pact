...
class Pact(nn.Module):
    ...
    def _perturb_latents(self, gen_latents):
        ...
        # swap mode is handled directly in _generate_latents_iteratively,
        # not here. Fall through as no-op for any unknown mode.
        return gen_latents
    ...
    def _generate_latents_iteratively(self, input_ids, attention_mask, inputs_embeds, position_ids):
        ...
        for iteration in range(num_iterations):
            ...
            # For swap: save B's real latents but don't modify them.
            # B generates normally using its own latents at every stage.
            if use_swap:
        ...
        # --- Swap: replace ALL latents at once AFTER normal generation ---
        if use_swap:
            if self._swap_buffer is not None:
                # Overwrite every latent position with the previous example's latents
                for iteration, swapped_latents in enumerate(self._swap_buffer):
                    ...
                    for j in range(self.n_latents):
                        ...
                        inject_mask = latent_mask[:, target_pos]
                        if inject_mask.any():
                            working_embeds[inject_mask, target_pos, :] = swapped_latents[inject_mask, j, :]

                # Re-run the transformer over prefix+latents to rebuild the KV
                # cache so it is consistent with the swapped embeddings.
                outputs_rerun = self.base_causallm(
                    inputs_embeds=working_embeds[:, :current_pos, :],
                    attention_mask=custom_attention_mask[:, :, :current_pos, :current_pos],
                    position_ids=position_ids[:, :current_pos],
                    output_hidden_states=True,
                    use_cache=True
                )
            ...
            # Rotate: this example's real latents become the buffer for the next.
